...
func TestGetArticleList(t *testing.T) {
	...
	{
		// basic + order
		list, count, err := GetArticleList(db, 1, 10, "", "", "", 0, 0)
		assert.Nil(t, err)
		assert.Equal(t, 3, count)
		assert.Equal(t, "test2", list[0].Title) // is_top desc

		// search category_id
		list, count, err = GetArticleList(db, 1, 10, "", "", "", 0, 3)
		assert.Nil(t, err)
...

	{
		...
		list, count, err := GetArticleList(db, 1, 10, "", "", "", 1, 0)
		assert.Nil(t, err)
		assert.Equal(t, 1, count)
...
func TestSaveOrUpdateArticle(t *testing.T) {
	...
	// save
	{
		...
		_, count, err := GetArticleList(db, 1, 10, "", "", "", 0, 0)
		assert.Nil(t, err)
		assert.Equal(t, 1, count)
